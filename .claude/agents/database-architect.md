...
-- Set the current tenant per transaction (never session-wide -- see the note below)
...
-- Application role enforces RLS. It must be able to connect, must NOT own the
-- tables, and must NOT be a superuser -- see the note below.
CREATE ROLE app_user LOGIN PASSWORD 'set-me-from-a-secret';
GRANT SELECT, INSERT, UPDATE, DELETE ON projects, tasks TO app_user;

-- Set tenant context per request, inside a transaction so it cannot outlive it:
-- BEGIN; SELECT set_config('app.current_tenant', $1, true); ...queries...; COMMIT;
```

Two failure modes make RLS silently inert. Both were reproduced on PostgreSQL 17:

- **The connecting role bypasses the policy.** A table's owner bypasses RLS unless
  the table is set to `FORCE ROW LEVEL SECURITY`, and a **superuser bypasses it
  unconditionally -- `FORCE` does not help there**. Default container setups connect
  as the `POSTGRES_USER`, which `initdb` makes a superuser (`rolsuper`,
  `rolbypassrls`), so every policy above would be dead weight while `pg_policies`
  still reports the setup as correct. This is why `app_user` exists: it must be a
  distinct, non-owning, non-superuser role, and migrations keep using the owner.
- **A session-scoped `SET` leaks across pooled connections.** `SET app.current_tenant`
  persists on the physical connection after the request ends, so the next request to
  reuse it inherits the previous tenant. Use `set_config(..., true)` (transaction-local)
  inside an explicit transaction that also pins the connection. Measured on a pool of 4
  with 60 concurrent requests: the session-scoped form served the wrong tenant 46 times
  out of 60; the transaction-scoped form, 0.

`current_setting('app.current_tenant')` without a second argument raises an error when
the variable is unset -- fail-closed, but as a database exception. Pass `true` as the
second argument to get NULL instead, which yields zero rows.
